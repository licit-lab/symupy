...
def render(network, fig=None, termination_zone=[], sensors=[], path={}):
    ...
    for id, link in network.links.items():
        network_coords.append([link['upstream_coords'].tolist()] + [arr.tolist() for arr in link['internal_points']] + [link['downstream_coords'].tolist()])
    ...
    network_coords = np.delete(np.array(network_coords, dtype=object), list(ids_to_delete), axis=0)
    network_coords = np.row_stack([arr+[[None, None]] for arr in network_coords])
    fig.gca().plot(network_coords[:, 0], network_coords[:, 1], 'k', label='Network', alpha=0.7, linewidth=1)
