...
def test_monitorVEH_distance():
    monitor = SymuviaMonitorVEH([0,1], "distance")
    res = monitor.update(6, _test_string_0, 0)
    assert res == (6, 0)
    res = monitor.update(6, _test_string_1, 0)
    assert res == (6, 13.998660650031834)


def test_monitorMFD():
    monitor = SymuviaMonitorMFD()
    res = monitor.update(6, _test_string_0, None)
    assert res == (3, 127.0)


def test_monitorAccumulation():
    monitor = SymuviaMonitorAccumulation()
    res = monitor.update(6, _test_string_0, None)
    assert res == (6, 3)


def test_monitorTTT():
    monitor = SymuviaMonitorTTT(['Rue_Crequi_SN_1'])
    res = monitor.update(6, _test_string_0, None)
    res = monitor.update(7, _test_string_1, None)
    assert res == (7, 2)


def test_monitorTTD():
    monitor = SymuviaMonitorTTD(['Rue_Crequi_SN_1'])
    res = monitor.update(6, _test_string_0, None)
    res = monitor.update(7, _test_string_1, None)
...
def test_monitorFlux():
    monitor = SymuviaMonitorFlux(['Rue_Duguesclin_NS_1'])
    res = monitor.update(6, _test_string_0, 0)
    res = monitor.update(7, _test_string_1, 0)
    assert res == (7, 1)


def test_monitorFlow():
    monitor = SymuviaMonitorFlow(['Rue_Duguesclin_NS_1'])
    res = monitor.update(6, _test_string_0, 0)
